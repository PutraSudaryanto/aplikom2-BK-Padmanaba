...
<table class="table table-striped table-bordered table-hover" width="100%" border="1">
  ...
  `tingkat_kelas`,
  `status_kelas`
FROM `sma3_bk`.`kelas` order by id_kelas asc");
	?>
  ...
  <tr>
    ...
    <td>
    <?php 
	$q=mysql_query("SELECT siswa.nis
, siswa.nama
 FROM 
 siswa WHERE siswa.nis not in(select kelas_siswa.nis from kelas_siswa where kelas_siswa.id_tahun_ajaran='".$_SESSION['id_tahun_ajaran']."')
");
